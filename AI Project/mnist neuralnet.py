# -*- coding: utf-8 -*-
"""
Created on Fri Nov 23 22:52:41 2018
...
START_TIME = time.time() # 현재 시간 측정(UTC 기준)
ft = gzip.open('data_training', 'rb') #파일 열어 읽기 모드로.
TRAINING = pickle.load(ft) #피클로 로딩
ft.close()
ft = gzip.open('data_testing', 'rb')
TESTING = pickle.load(ft)
ft.close()

print('Import duration '+str(round((time.time() - START_TIME), 2))+'s') #이미지 로딩시간 출력
print('----')
...
nt1=Network(300) # 은닉층 노드로 300개 생성
nt1.train(600,TRAINING) # 배치 크기 600, training data 입력
